...
use crate::utils::{calculate_month_id, increase_account_size, validate_score};
...
pub struct ApproveResult<'info> {
    ...
    #[account(
        constraint = campaign_state.influencer.eq(&influencer.key()) @ ErrorCode::CampaignInfluencerMismatch
    )]
...
pub fn handle(ctx: Context<ApproveResult>, campaign_id: u64, score: u64) -> Result<()> {
    ...
    require!(validate_score(score), ErrorCode::InvalidScoreValue);

    let (reward_scale_count, reward_scales) =
        global_state.get_reward_scales(ScaleType::ApproveResults);

    for i in 0..reward_scale_count {
        let idx = i as usize;
        if score >= reward_scales[idx].bound {
            campaign_state.rating += reward_scales[idx].value;
            break;
        }
    }

    campaign_state.status = CampaignStatus::ResultAproved;
    ...
    if !global_point_info.try_update(month_id, boosted_rating) {
        increase_account_size(
            ...
            &mut ctx.accounts.brand.to_account_info(),
